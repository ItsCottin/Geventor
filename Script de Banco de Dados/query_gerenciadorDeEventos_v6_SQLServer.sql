...
DELIMITER //
CREATE PROCEDURE IS_EXIST_INSCRITO_IN_EVENTO(IN idEven BIGINT, INOUT outResult bit)
BEGIN
    DECLARE QTD INT; 
	DECLARE outResult bit; 
    SELECT COUNT(*) INTO QTD FROM TBL_USUARIO_ATIVIDADE WHERE STATUS IN ('Pendente', 'Aprovado') AND ID_ATIVI IN 
    (SELECT ID_ATIVI FROM TBL_ATIVIDADE WHERE ID_EVEN IN 
    (SELECT ID_EVEN FROM TBL_EVENTO WHERE ID_EVEN = idEven));
    IF (QTD > 0) THEN
        SET outResult = 1;
    ELSE
        SET outResult = 0;
