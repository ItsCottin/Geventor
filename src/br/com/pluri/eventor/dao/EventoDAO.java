...
public interface EventoDAO extends BaseDAO<Evento> {
	...
	@Query(value = "select * from TBL_evento where ID_USUA != :idUsu and DATAINICIO_EVEN > GETDATE()", nativeQuery = true)
	public List<Evento> findAllEventoMenosMeusRecen(@Param("idUsu") Long idUsu);
	
	@Query(value = "SELECT * FROM TBL_EVENTO WHERE ID_USUA != :idUsu AND DATAINICIO_EVEN > GETDATE() AND EXISTS (SELECT ID_EVEN FROM TBL_ATIVIDADE WHERE ORGANIZACAO_ATIVI = :tpAtiv AND TBL_ATIVIDADE.ID_EVEN = TBL_EVENTO.ID_EVEN)", nativeQuery = true)
	public List<Evento> findAllEventoMenosMeusComAtivExistPorTipoAtivRecen(@Param("idUsu") Long idUsu, @Param("tpAtiv") String tpAtiv);
	
	@Query(value = "select MIN(datainicio_even) as proximoevento from tbl_evento where datainicio_even >= GETDATE() and id_usua = :idUsu",
			nativeQuery = true)
